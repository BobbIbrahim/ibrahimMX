...
public class AutomationSchedulerService {
	 ...
	 * @param startPaused
	 *            whether the schedule should be created in a paused state
	 */
	...
	public AutomationScheduleDescription describeSchedule(String temporalScheduleId) {
		...
		return AutomationScheduleDescription.builder().paused(paused).nextRunAt(nextRunAt).lastRunId(lastRunId).build();
	}
	...
	public void updateSchedule(Automation automation) {
		...
		handle.update(input -> {
			Schedule updated = Schedule.newBuilder(input.getDescription().getSchedule()).setAction(action).setSpec(spec)
					.build();
	...
	/**
	 * Pauses a schedule. {@code note} may be null or blank for the SDK default
	 * note.
	 */
	...
	/**
	 * Resumes a schedule. {@code note} may be null or blank for the SDK default
	 * note.
	 */
	...
	/**
	 * Deletes a Temporal schedule. A schedule that is already gone is treated as a
	 * successful delete; any other Temporal failure is rethrown.
	 */
	...
	private ScheduleSpec weeklySpec(Automation automation) {
		...
		// Stored weeklyDay uses Monday=1. Sunday=7. Temporal's calendar day-of-week
		// uses
		// Sunday=0. Saturday=6. Monday(1) through Saturday(6) already line up between
		// the
		// two numbering systems, so only the stored Sunday value (7) needs remapping to
		// 0.
		int temporalDayOfWeek = automation.getWeeklyDay() == STORED_SUNDAY
				? TEMPORAL_SUNDAY
	...
	private ScheduleCalendarSpec.Builder calendarSpecBuilder(Automation automation) {
		...
		return ScheduleCalendarSpec.newBuilder().setHour(List.of(new ScheduleRange(automation.getRunTime().getHour())))
	...
	private boolean isNotFound(ScheduleException exception) {
		...
		while (current != null) {
			if (current instanceof StatusRuntimeException statusException
					&& Status.Code.NOT_FOUND.equals(statusException.getStatus().getCode())) {
				return true;
			}
