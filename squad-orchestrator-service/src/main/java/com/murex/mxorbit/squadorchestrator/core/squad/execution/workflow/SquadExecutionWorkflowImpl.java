...
public class SquadExecutionWorkflowImpl implements SquadExecutionWorkflow {
	...
	public SquadExecutionResult execute(SquadExecutionRequest request) {
		Squad squad = getSquadActivity.getSquadById(request.getSquadId());
	...
	private SquadStep findFirstStep(Squad squad, Set<String> targetedStepIds) {
		List<SquadStep> sourceCandidates = squad.getSteps().stream()
				.filter(step -> !targetedStepIds.contains(step.getId())).sorted(Comparator.comparing(SquadStep::getId))
				.toList();
		...
		if (sourceCandidates.isEmpty()) {
			throw new IllegalStateException(
					"Sequential squad execution requires one starting step, but none was found.");
		}

		throw new IllegalStateException(
				"Sequential squad execution requires one starting step, but found " + sourceCandidates.size() + ".");
	}
